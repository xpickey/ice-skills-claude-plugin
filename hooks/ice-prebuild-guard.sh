...
if [[ "$CMD" == *ICE_BUILD=pipeline* ]] && writes_office_file && ! edits_system_files; then
  ...
  if [[ -n "$SID" && -f "$HOME/.claude/hooks/ice_route_lib.py" ]]; then
    ...
    if [[ "$RC" -eq 3 ]]; then
      deny "ยังสร้างไฟล์ไม่ได้ เพราะขั้น \"โหลดก่อนคิด\" ที่ตารางเส้นทาง skill กำหนดไว้ยังไม่ครบ — $MISSING — ให้โหลด skill ด้วย Skill tool และเปิดอ่านไฟล์ที่ระบุก่อน แล้วรันคำสั่งนี้อีกครั้ง (ตารางอยู่ที่ ~/.claude/hooks/skill-routing.yaml)"
    fi
  fi
  exit 0
fi
