...
def main():
    ...
    # V01R02 (2026.09.06 · คำถามของ user "ทำไมเห็นแล้วไม่โหลด"): ข้อความเดิมเป็นคำบอกกลาง ๆ จึงถูกข้ามได้เมื่อกำลังทำงานต่อเนื่อง
    # ตอนนี้ถ้ายังโหลดไม่ครบ ให้ขึ้นบรรทัดสั่งงานเป็นบรรทัดแรกสุด เพื่อให้เป็นสิ่งแรกที่อ่านก่อนเริ่มคิด
    lines = []
    if ms or mr:
        todo = []
        if ms:
            todo.append("โหลด skill: " + ", ".join(ms))
        if mr:
            todo.append("เปิดอ่าน: " + ", ".join(os.path.basename(p) for p in mr))
        lines.append("สิ่งที่ต้องทำก่อนตอบข้อความนี้ — " + " · ".join(todo) +
                     " · ทำให้ครบก่อนเริ่มคิดหรือออกแบบ ไม่ใช่ก่อนสร้างไฟล์ (ด่านเครื่องจะปฏิเสธการเขียนไฟล์ผลงานและการแจกงานให้เพื่อนร่วมทีมจนกว่าจะครบ)")
