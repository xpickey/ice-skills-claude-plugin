...
> **Engine guideline (Compass ใส่ใน brief ให้เจนนี่เลือก) — Higgsfield = หลัก, gemini = เสริม:** ค่าเริ่มต้นทุกงานภาพ/วิดีโอ (hero / 4K / video / ad / brand / character / animate) → **higgsfield** (credit-based 208 starter — preflight cost ก่อนงานแพง). **ใช้ gemini (rlabs) เป็นตัวเสริมเฉพาะเมื่อ** ต้องภาพภายในเร็ว/ร่าง ที่ไม่ต้อง 4K + อยากประหยัด credit (quota Google) หรือ multi-turn image-edit (session-based) — MCP เสมอ ทุก env (binary local). Execution path เป็นของเจนนี่: Claude Code (มี Bash) + Higgsfield → CLI (`hf generate create`); Claude Desktop/Web/Cowork (ไม่มี shell) + Higgsfield → MCP tool; gemini (rlabs) → MCP เสมอ (`mcp__gemini__gemini-generate-image` · edit ภาพ = session-based: `gemini-start-image-edit` → `gemini-continue-image-edit` → `gemini-end-image-edit`).
...
section_pack:
  ...
  cb_unit_spec:       # ⭐ CB เท่านั้น (Composed Build §) — 1 spec ต่อ 1 หน่วย (หน้า/บท) ส่งให้แจนนี่ทีละหน่วย
    unit_id: "<page-N | chapter-N>"
    unit_type: "<title|content|compare|pricing|exec-summary|...>"
    position: { index: N, of: TOTAL }
    frame_ref: { template, theme_font_strategy, layout_archetype }   # lock จาก Phase 0 — แจนนี่ห้ามเบี่ยง
    build_scope: "<preview-single | final-batch>"                    # preview = Phase 3 · final = Phase 4
    content: "<verified content หน่วยนี้>"
    reviewer_verdict: "<consolidated accept/reject จากกัปตัน — ONE verdict ไม่ใช่หลาย stream>"
...
**⭐ GLOSSARY 4-CAP (อย่าสับสน 4 ตัวนี้):**
...
- **PUL CAP** (Per-Unit Loop, CB เท่านั้น) = รอบแก้ต่อ 1 หน่วย (หน้า/บท) Fast=1 · Full=2 · Submit=3 · ครบ → STOP+escalate (ก accept / ข เปลี่ยนทิศ / ค ข้าม). **คนละแกนกับ CHAIN-ROUND:** frame-recheck → re-entry Phase-1 กิน 1 CHAIN-ROUND + reset PUL · revisit หน่วยใน Phase-2 ไม่ reset PUL · re-frame ทั้งงาน cap=1/run · >⅓ หน่วยชน PUL cap → STOP+frame-recheck
...
**id15 Composed Build (CB)** = multi-unit deck (>10 slides) / proposal (≥2 บท) · Pattern **#4 Generate-And-Filter (capped per-unit filter loop)** — CB = composition over #4/#2/#3 (#4-outline → #2-fanout reviewer → #3-verify FACT-gate → #4-filter per-unit build+inspect แบบมี cap → #4-assemble). per-unit loop = **#4-filter ที่มี hard cap ไม่ใช่ #6 Loop-Until-Done** → invariant "ไม่มี #5/#6" ยังอยู่. CB ใช้แทน id1/id3/id10 เฉพาะเหนือ threshold (ดู Composed Build §)
...
# ⭐ Composed Build — Outline-First → Per-Unit-Review → Build-Once (CB · Matrix id15 · Pattern #4)

**เมื่อไร:** deck **>10 slides** หรือ proposal **≥2 บท** (customer-facing). **≤10 slides & <2 บท → CB OFF** ใช้ single-pass เดิม (id1/id3/id10). กัปตัน = design authority + frame-keeper + dispatcher + final inspector ตลอดทาง.
**Track A** unit=หน้า/slide · **Track B** unit=บท/chapter — engine เดียว parameterise ด้วย unit.

**5 Phase:**
```
Phase 0 FRAME    — กัปตันกำหนดกรอบ: register (Professional-B2B), template/theme, น้ำเสียง, term_policy (Card B6), audience
Phase 1 OVERALL  — หารือ "ภาพรวม/โครง" กับ ท่านเทพ ก่อน (FACT-gate) — ❗ไม่ batch ไม่ข้าม. ได้ outline + frame_ref lock
Phase 2 PER-UNIT — ทีละหน่วย (ตาม Granularity Ladder): draft + หารือ reviewer (ท่านเทพ XOR Codex by content)
Phase 3 PREVIEW  — แจนนี่ build หน่วย/section → render preview → กัปตัน inspect กรอบ
                   ตรง → accept · ไม่ตรง → feedback แก้ (PUL CAP Fast1/Full2/Submit3) · ครบ cap → escalate User
Phase 4 BUILD-ONCE — ครบทุกหน่วย accept → ส่งแจนนี่ build รวมทีเดียว จาก accepted unit-specs (ไม่ใช่ stitch preview fragments → กัน §13 corrupt)
Phase 5 FINAL INSPECT — กัปตันตรวจ artifact เดิม (อันที่ Phase 4 build): PASS → present เลย (❌ไม่ build ซ้ำ) · FAIL → rebuild (QA-REBUILD CAP=2)
```

**Granularity Ladder (กัน token blowup):** N = จำนวน unit
- `N ≤ 12` → true per-unit (ทุกหน่วย preview แยก)
- `13–30` → section-batch (≤6 units/section · 1 preview/section)
- `N > 30` → section-batch + **SAMPLE-FRAME** (1 representative unit/section เซ็ต pattern · ที่เหลือ inherit batch-build ไม่ preview เดี่ยว) → deck 77 ≈ 8 preview ไม่ใช่ 77
- Phase 1 overall outline **ไม่ batch เด็ดขาด**

**ALWAYS-DRILL (≤8 ceiling):** หน่วย commitment (pricing/man-day/TOR-compliance/exec-summary/commercial table) ตรวจเดี่ยวเสมอ — แต่ cap ≤8 หน่วย · เกิน → รวมเป็น 1 commitment-section drill ครั้งเดียว (กัน TOR deck re-expand)

**Per-Unit Reviewer Router (XOR by content):**
...
**preserve:** Hard Delegation (กัปตันไม่ build inline — แจนนี่ build) · Producer≠Frame-Inspector≠Checker (แจนนี่ build · กัปตัน frame-inspect · เจ้ระเบียบ QA final post-build) · CB ส่ง **cb_unit_spec** (frame_ref lock + build_scope: preview-single|final-batch) ต่อหน่วย ผ่าน section_pack.
