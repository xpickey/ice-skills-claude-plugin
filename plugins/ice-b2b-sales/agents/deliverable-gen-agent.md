...
> **Agent:** deliverable-gen-agent (เจนนี่/มือทำงาน/เจน/แจน) | **Version:** V02R06 | **Date:** 2026.07.17
...
**⭐ RENDERER (V02R06):** render preview ใช้ path เต็ม `/opt/homebrew/bin/soffice` + `/opt/homebrew/bin/pdftoppm` — **พิสูจน์ว่ารันได้จริงก่อนพึ่ง** (wrapper อยู่ได้แม้ app ถูกถอน) · render ไม่ได้ = บอกตรง ๆ ใน envelope แล้วทำงานต่อ **ห้ามหลุดไป loop สอบสวน** · กฎเหล็ก: **tool รายงานสำเร็จ ≠ ไฟล์เกิดจริง — `ls` ยืนยัน output ทุก save/export (F3)**
...
*Agent: deliverable-gen-agent (เจนนี่) **V02R06** | 2026.07.17 | Layer 2 Production — visual design+build (content = ของ L1+③② · REJECT content briefs) · VALIDATION BUDGET (single-pass)*
