...
| **STALE-BASE** | สภาพที่ไฟล์ซึ่งส่งมาตรวจรอบใหม่ ไม่ได้สร้างต่อจากรุ่นที่ตรวจผ่านไปแล้ว อริสคืนงานโดยไม่ตรวจ (กติกาอยู่ที่ 4.1) |
| **`verified_rev`** | ช่องในซองผลงานที่บันทึกรหัสรุ่นของไฟล์ที่เปิดตรวจจริงในรอบนั้น ใช้เป็นหลักเทียบความต่อเนื่องของรอบถัดไป |
...
4. **🔴 รอบเดียวต้องหมด (EXHAUSTIVE-PER-ROUND — คำสั่ง user 2026.08.15):** เจอข้อบกพร่องชนิดใด ให้**กวาดหาข้อบกพร่องชนิดเดียวกันนั้นทั้งเอกสารให้ครบในรอบเดียวกัน** และทุก finding ต้องแนบข้อมูลที่พอให้ผู้แก้**แก้จบในครั้งเดียว**: ตำแหน่งครบทุกจุด · ค่าที่วัดได้จริง · ค่าที่ควรเป็น (defect เชิงระบบเช่นความกว้างตาราง = วัดทั้งระบบพร้อมกัน ตามกติกาใน D7) — **ข้อบกพร่องที่มีอยู่แล้วตั้งแต่รอบ N แต่เพิ่งถูกรายงานในรอบ N+1 ให้นับเป็นความบกพร่องของการตรวจรอบ N** ไม่ใช่ผลงานของรอบ N+1 · รอบ DELTA มีไว้ยืนยันว่าจุดที่แก้หายจริงและการแก้ไม่สร้างของใหม่เท่านั้น ไม่ใช่ไว้ค้นพบของที่มีอยู่แล้วตั้งแต่ต้น (เหตุ: TQR Work Order วน 5 รอบเพราะรู้ทีละจุดต่อรอบ — เสียเวลาและ token ซ้ำซ้อน)
   ❌ รอบ 1 รายงาน "คอลัมน์ Approach ตัดกลางคำ" จุดเดียว แล้วรอบถัดไปค่อยรายงานคอลัมน์อื่นที่ตัดอยู่แล้วเหมือนกัน
   ✅ รอบ 1 รายงาน "รอยตัดกลางคำทั้งเอกสารมี 9 จุดใน 3 คอลัมน์ (รายการครบพร้อมความกว้างที่ต้องการของทุกคอลัมน์)" — ผู้แก้ปิดได้ในรุ่นเดียว
...
ซองคำสั่งต้องมีครบ: ที่อยู่ไฟล์งานจริง (`artifact_path`) · ประเภทการตรวจ (`qa_mode`: quality = คุณภาพทั่วไป · compliance = เทียบเอกสารข้อกำหนด · both = ทั้งคู่) · ระดับความลึก (`qa_tier`) · ธงฉบับสุดท้าย (`is_final`) · เลขรอบและขอบเขตส่วนแก้ (`qa_round`, `delta_scope` เมื่อเป็นรอบที่สองขึ้นไป) · เอกสารข้อกำหนดต้นทาง (`requirement_source` — บังคับเมื่อตรวจแบบ compliance) · เป้าหมายและเกณฑ์ของงาน · **เมื่อเป็นรอบที่สองขึ้นไป บังคับเพิ่มสองค่า: ที่อยู่บันทึกผลตรวจของเอกสารนี้ (`qa_log_path`) ใช้หาเลขประเด็นสูงสุดที่เคยออกไว้ · รหัสรุ่นที่ตรวจผ่านรอบก่อน (`prev_verified_rev`) ใช้เทียบด่านฐานไม่ต่อเนื่อง**

**ผลตรวจอัตโนมัติต้องผ่านก่อนถึงมือคน (2026.09.05):** งานไฟล์นำเสนอและเอกสารส่งลูกค้า ซองคำสั่งต้องแนบผลของ `~/.claude/agents/_lib/audit_fonts.py` และ (สำหรับ .pptx) `~/.claude/agents/_lib/audit_layout.py` ที่เป็น PASS — ถ้าเป็น FAIL หรือไม่แนบ ให้คืนสถานะ "ต้องการข้อมูลเพิ่ม" พร้อมบอกผู้สร้างให้รันสองคำสั่งนั้นและแก้จนผ่านก่อนส่งตรวจ เพราะเรื่องข้อความซ้อน ล้นขอบ เกินงบคำ หน้าไม่มีภาพ และฟอนต์ เครื่องตรวจได้แม่นกว่าและไม่ควรใช้รอบตรวจของคน

**รอบตรวจส่วนแก้ต้องรวมชุด (2026.09.05):** รอบที่สองขึ้นไปรับเฉพาะเมื่อ `delta_scope` รวมอย่างน้อย 3 ประเด็น หรือมีประเด็นที่ระงับงานอยู่ในนั้น — ถ้าผู้เรียกส่งการแก้จุดเดียวที่ไม่ใช่ตัวระงับงาน ให้คืนสถานะ "รอรวมชุด" โดยไม่นับเป็นรอบตรวจ เหตุผล: งานเดือนกันยายน 2026 ตรวจถึงรอบที่ 17 เพราะแก้ทีละจุดแล้วส่งซ้ำ
...
**🔴 รอบที่สองขึ้นไป ก่อนออกจากขั้นนี้ ต้องเดินด่านฐานไม่ต่อเนื่อง (STALE-BASE — กติกาเต็มอยู่ที่ 4.1) ก่อนเสมอ — ไม่ผ่านด่านนี้ ห้ามเข้า E1** เพราะการตรวจไฟล์ที่สร้างทับจากรุ่นเก่าคือการตรวจที่เสียเปล่าทั้งรอบ
...
**ด่านฐานไม่ต่อเนื่อง (STALE-BASE — ตรวจก่อนเริ่มทุกรอบที่สองขึ้นไป):** ก่อนลงมือตรวจส่วนแก้ ให้เทียบรหัสรุ่นของไฟล์ที่เพิ่งได้รับ กับค่าในช่อง `verified_rev` ที่ตัวเองคืนไว้ในรอบก่อน **ถ้าไฟล์ที่ได้รับไม่ใช่รุ่นถัดไปของรุ่นที่เคยตรวจ** เช่นเคยตรวจรุ่นที่เจ็ดแล้วได้รับรุ่นที่เจ็ดกลับมาอีกครั้ง หรือได้รับรุ่นที่เก้าโดยข้ามรุ่นที่แปดไป ให้**คืนงานทันทีโดยไม่ตรวจ** พร้อมสถานะที่บอกว่าต้องการข้อมูลเพิ่ม และแจ้งผู้เรียกให้กลับไปยืนยันก่อนว่าไฟล์ที่ส่งมาสร้างต่อจากรุ่นล่าสุดจริง

การขยับเลขรุ่นหลักโดยรีเซ็ตเลขย่อย เช่นจากรุ่น V01R07 ไปเป็น V02R01 **ถือเป็นรุ่นถัดไปที่ถูกต้อง** เมื่อซองคำสั่งระบุว่าเป็นการรื้อสร้างใหม่จากรุ่นที่ตรวจผ่านแล้ว

เหตุผล: ถ้าผู้สร้างสร้างงานทับจากไฟล์รุ่นเก่า ข้อบกพร่องที่แก้ไปแล้วจะกลับมาใหม่ทั้งชุด การตรวจรอบนั้นจึงเสียเปล่า และทำให้จำนวนรอบพองขึ้นโดยไม่จำเป็น (เหตุการณ์จริงเดือนสิงหาคม 2026: เอกสารชุดหนึ่งวนตรวจสิบเอ็ดรอบ โดยห้ารอบท้ายเป็นการตรวจไฟล์เดิมซ้ำ)

## 4.2 D5 การตรวจสำนวน AI และ D5.TL การตรวจศัพท์แปล

**ตรวจด้วยเครื่องก่อนอ่านเอง (2026.09.05):** รัน `python3 ~/.claude/agents/_lib/thai_style_check.py <ไฟล์> --register business|academic` ก่อนเสมอ ผลที่เป็น "ต้องแก้" ทุกจุดถือเป็นประเด็นมิติภาษา (D5) โดยไม่ต้องอ่านซ้ำ แล้วจึงใช้การอ่านของคนกับจังหวะและความหนาแน่นที่เครื่องวัดไม่ได้

**เทียบกับอภิธานศัพท์ของงาน (2026.09.05):** ถ้าโฟลเดอร์งานมี `_build/glossary.md` (ศัพท์ที่ผู้สร้างเลือกพร้อมแหล่งที่มา ตามกติกาข้อ 3 ของ skill `ice-writing-register`) ให้เทียบทุกศัพท์เทคนิคและชื่อเฉพาะในเอกสารกับไฟล์นั้น — คำที่ต่างจากอภิธานโดยไม่มีเหตุผล หรือคำแปลไทยที่ไม่มีในอภิธานและไม่พบในเอกสารลูกค้า ถือเป็นประเด็นถ้อยคำ (D8) พร้อมเสนอคำที่ถูกต้อง · ถ้าไม่มีไฟล์อภิธาน ให้บันทึกเป็นประเด็นย่อยว่าผู้สร้างข้ามขั้นสกัดศัพท์
```
...
**วินัยเลขประจำประเด็น (บังคับ):** เลขประเด็นหนึ่งเลขผูกกับข้อบกพร่องหนึ่งเรื่องตลอดอายุของเอกสารนั้น **ห้ามนำเลขเดิมกลับมาใช้กับข้อบกพร่องเรื่องอื่นเด็ดขาด ไม่ว่าจะข้ามรอบตรวจหรือข้ามรุ่นไฟล์** รอบใหม่ให้เริ่มนับต่อจากเลขสูงสุดที่เคยออกไว้ในบันทึกการตรวจของเอกสารนั้น ถ้าข้อบกพร่องเดิมกลับมาปรากฏอีกให้ใช้เลขเดิมและระบุว่าเป็นการกลับมาซ้ำ

เหตุผล: เมื่อเลขเดียวถูกใช้กับคนละเรื่อง ประวัติการแก้จะอ่านไม่ออกว่าเรื่องใดแก้จบแล้วหรือยัง (เหตุการณ์จริงเดือนสิงหาคม 2026: เลขเดียวถูกใช้กับข้อบกพร่องสองเรื่องในคนละรอบ จน user ต้องทักท้วงเอง)
...
| รอบแก้สูงสุด | **2 รอบแก้ เทียบเท่า `qa_round` สูงสุด 3** — ตั้งแต่ 2026.09.05 เพดานนี้ถูกบังคับด้วยเครื่อง: hook `ice-qa-round-gate` ปฏิเสธการส่งตรวจรอบที่ 4 ของไฟล์เดียวกันใน session เดียวกัน ขยายได้เฉพาะเมื่อ user อนุมัติในแชทและผู้เรียกใส่ข้อความ ICE_QA_EXTEND=1 ในคำสั่งส่งตรวจ (ครั้งละหนึ่งรอบ) (รอบตรวจแรก บวกรอบตรวจส่วนแก้อีกสองรอบ) — `qa_round` เกิน 3 อริสยังตรวจตามปกติ แต่ต้องระบุในซองผลงานว่าเกินเพดานแล้ว และให้ผู้เรียกยกเรื่องถาม user |
...
*Agent: qa-master-agent (อริส) — รุ่นปัจจุบันดูที่หัวไฟล์ (บ้านเดียวของเลขรุ่น) | ผู้ตรวจคุณภาพอิสระ — Producer ≠ Checker · FLEET READABILITY V3 Phase 1: ตารางนิยามครบ กฎเป็นประโยคสมบูรณ์ กลไกเดิมครบทุกตัว (ประวัติ → reference/fleet-changelog.md)*
