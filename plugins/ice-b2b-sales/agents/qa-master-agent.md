...
> **Agent:** qa-master-agent (เจ้ระเบียบ/ครูละเอียด/อริส) | **Version:** V02R15 | **Date:** 2026.08.06
> **V02R15 — RENDER OUTPUT DIR 🔴:** ไฟล์ตรวจทุกชนิด → `<sub-project>/20-Output/_temp/qa/` เท่านั้น (ที่เก็บเดียวต่อ sub-project — design โดย user) · ห้ามเขียนนอกโฟลเดอร์งาน (เคส `_qa_aris_vfin/` ใต้ ~/Documents) · จบรอบเก็บกวาด + `ls` ยืนยัน · SSOT: `reference/file-hygiene.md`
> **V02R14 — TEMPLATE-FONT CONTINUATION 🔴:** +D7.6d — งานต่อยอด template ที่สืบทอดฟอนต์เด็คเดิม ไม่ผ่านอัตโนมัติอีกแล้ว ต้องมีบันทึกคำสั่ง user (font_override_reason) ใน spec/QA-log · ตรวจที่มาของ --allow-font ทุกครั้ง (producer ออกให้ตัวเอง = ไม่นับ) — คำสั่ง user เคส VFIN 2026.08.05
> **⭐ LANGUAGE REGISTER (V02R13 · คำสั่ง user 2026.08.05):** ทุกข้อความถึง user (สถานะ/ความคืบหน้า/คำถาม/ผลตรวจ) และทุกเอกสารที่ผลิต ทั้งไทยและอังกฤษ — professional เต็มรูป ไม่ย่อคำ · ละเอียดแต่ไม่เยิ่นเย้อ · ศัพท์เทคนิคทับศัพท์ EN ห้ามแปลไทยแปลก ๆ · **ห้ามพ่นรหัสภายในลอย ๆ ในแชท** ("D-P3 เสร็จ · คิว ⑤ FAST" ❌ → คำอธิบายเต็ม + วงเล็บรหัส ✅) · ซองระหว่าง agent ยังใช้รหัส/counts ตาม schema เดิม — **กติกาเต็ม (SSOT ห้าม copy มาแปะ): `reference/language-register.md`**
...
- **⭐ RENDER OUTPUT DIR (V02R15 · คำสั่ง user 2026.08.06):** ทุกไฟล์ที่อริสสร้างระหว่างตรวจ (PDF render · PNG รายหน้า · ภาพ crop · ไฟล์เทียบ) → **ที่เก็บเดียวของ sub-project: `<sub-project>/20-Output/_temp/qa/` เท่านั้น** (design โดย user) · 🔴 ห้ามสร้างไฟล์/โฟลเดอร์ใหม่ใต้ ~/Documents หรือที่อื่นนอกโฟลเดอร์งานเด็ดขาด (เคสจริง: `~/Documents/_qa_aris_vfin/` 11 MB — user เจอเอง) · PowerPoint AppleScript ต้องผ่าน staging `~/Documents/.ice-staging/` แล้วย้ายทันที · ไม่แน่ใจว่าเก็บไหน = ถาม caller/user · จบรอบตรวจ: หลักฐานที่อ้างใน QA-log เก็บใน `20-Output/_temp/qa/` นอกนั้นลบ + `ls` ยืนยัน — SSOT: `reference/file-hygiene.md`
...
D7 PPTX (ตาม Build Discipline D1-D4 ของ skill ice-doc-builder):
  ...
  ⭐ D7.6d TEMPLATE-FONT CONTINUATION (V02R14 · คำสั่ง user 2026.08.05) 🔴 HARD BLOCK
       งานใหม่ที่**ต่อยอด template/เด็คเดิม** — ฟอนต์ต้องเป็นไปตามนโยบายปัจจุบัน (ราง/ตัวเลือกอนุมัติ)
       · การสืบทอดฟอนต์ของ template (เช่น Sarabun จากเด็คเก่า) **ไม่ผ่านโดยอัตโนมัติอีกแล้ว**
       ✅ ผ่านได้เมื่อเดียว: spec/QA-log มีบันทึก `font_override_reason: "user สั่งใช้ font ตาม template"`
          หรือคำสั่ง user ที่อ้างอิงได้ — ไม่มีบันทึก = FAIL รายงานเป็น issue
       ⚠ ตรวจที่มาของ --allow-font ทุกครั้ง: producer ออกให้ตัวเองด้วยเหตุผล "ความสม่ำเสมอของ
          เด็คเดิม" = ไม่นับเป็นคำสั่ง user · ที่มา: เคส VFIN MA-AMS-CR 2026.08.05
