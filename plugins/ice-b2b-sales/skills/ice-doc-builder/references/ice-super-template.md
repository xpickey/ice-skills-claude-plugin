...
## §4 สูตรสร้าง background — ⭐ บังคับทุก deck (คำสั่ง user 2026.09.06 · ปฏิบัติอย่างเคร่งครัด)

**กติกาบังคับ:** ทุก deck ต้องมีภาพพื้นหลังลายเส้นทองคล้ำเป็นของตัวเอง และ **ต้องสร้างใหม่ทุกครั้งตามหัวเรื่องของเอกสารชิ้นนั้น** — ห้ามหยิบไฟล์พื้นหลังของงานก่อนหน้าที่เป็นคนละหัวเรื่องมาใช้ซ้ำ และห้ามใช้ไฟล์พื้นหลังสำเร็จรูปกลางที่ตายตัว เหตุผล: พื้นหลังคือส่วนที่ทำให้เอกสารแต่ละชิ้นมีเอกลักษณ์ของเรื่องนั้น การใช้ภาพเดิมข้ามเรื่องทำให้ทุกเล่มหน้าตาเหมือนกันจนลูกค้าจำไม่ได้ว่าเล่มไหนเรื่องอะไร · **ข้อยกเว้นเดียว:** deck ใหม่ของลูกค้าเดิมที่หัวเรื่องเดียวกัน (เช่นฉบับปรับปรุงของข้อเสนอเดิม) ใช้ชุดเดิมได้

1. **motif มาจากสองชั้น — ชั้นแรกคือหัวเรื่องเอกสาร ชั้นรองคืออุตสาหกรรมลูกค้า:** อ่านหัวเรื่องจริงของงาน (เช่น การปิดงบการเงินรายเดือน · การวางแผนสินค้าคงคลัง · การยื่นข้อเสนอราคา) แล้วเลือกลายที่สื่อเรื่องนั้น จากนั้นจึงผสมกับ motif ประจำอุตสาหกรรมใน §3 ให้เข้ากัน · ถ้าหัวเรื่องกว้างจนไม่มีลายที่ตรง ให้ใช้ motif อุตสาหกรรมล้วน และบันทึกเหตุผลไว้ใน design spec · **เกณฑ์ตัดสินว่าลายสื่อหัวเรื่องจริงหรือไม่:** ผู้ที่ไม่ได้อ่านไฟล์กำหนดรูปแบบต้องดูภาพแล้วบอกได้ว่าเป็นเรื่องอะไร — เส้นโค้งนามธรรมที่ต้องอธิบายกำกับว่าหมายถึงรอบเวลา ถือว่ายังไม่สื่อ ให้เติมองค์ประกอบที่อ่านออกทันทีเข้าไปอีกหนึ่งอย่าง เช่นหน้าปัดนาฬิกาเส้นบางสำหรับเรื่องความเร็ว หรือแถบปฏิทินสิ้นงวดสำหรับเรื่องรอบบัญชี (ผลตรวจคุณภาพ 2026.09.06)
2. **สเปกการยิง (Higgsfield MCP):** model `nano_banana_pro` · `aspect_ratio` 16:9 · `resolution` 2k · **ตรวจเครดิตคงเหลือด้วย `balance` ก่อนยิงเสมอ** และแจ้ง user เมื่อค่าใช้จ่ายเกินที่ตกลงไว้
3. **PROMPT แม่แบบ (เปลี่ยนเฉพาะ {motif} + {โทนพื้น}):** `elegant minimal {โทนพื้น} background, thin muted dark-gold line art of {motif}, subtle, luxurious consulting style, lines occupy the right and bottom edges only, large clear empty area on the left for text, no text, no logo, 16:9`
4. **ยิงกี่ภาพ:** ต่อ deck หนึ่งชิ้นใช้ **2 ภาพ** motif เดียวกัน — **พื้นเข้ม** (deep navy) ใช้กับหน้าปก หน้าคั่น หน้าปิด · **พื้นขาว** (off-white ลายจางกว่า) ใช้กับหน้าเนื้อหา · ยิงเผื่อได้ไม่เกินชุดละ 2 ตัวเลือกต่อโทน แล้วเลือกเก็บโทนละ 1 · **ห้าม generate รายหน้า** (ต้นทุนบานและภาพในเล่มไม่นิ่ง)
5. **เก็บและต่อเข้ากับตัวสร้างไฟล์:** ย่อเป็น 2000×1125 บันทึกเป็น .jpg ที่ `<งาน>/_build/assets/bg/` แล้วประกาศที่อยู่ไฟล์ในไฟล์กำหนดเนื้อหาระดับ deck สองช่องคือ `background_dark` และ `background_light` — ตัวสร้างไฟล์ (`_lib/build_pptx.py`) จะวางภาพเต็มหน้าไว้หลังสุดใต้ทุกกล่องข้อความให้เอง ไม่ต้องวางเอง · ไม่ประกาศสองช่องนี้ = ได้พื้นไล่เฉดเปล่าของแม่แบบ ซึ่งถือว่ายังทำไม่ครบตามกติกานี้ · **ที่เก็บไฟล์มีผลต่อการตรวจ:** ตัวสร้างไฟล์เตือนเมื่อภาพที่ประกาศอยู่นอกคลังภาพของงานชิ้นนั้น เพราะเป็นสัญญาณว่ายืมภาพของเอกสารเรื่องอื่นมาใช้
5b. **ความจางของลาย (ตัวสร้างไฟล์ทำให้เอง — ผลตรวจคุณภาพ 2026.09.06):** กริดเนื้อหาของแม่แบบกินความกว้างถึง 12.6 จาก 13.33 นิ้ว และวางส่วนท้ายหน้ากับเลขหน้าไว้ในแถบล่าง ซึ่งเป็นเขตเดียวกับที่ลายกระจุกอยู่ ลายที่ความเข้มเต็มจึงวิ่งผ่านหลังข้อความเสมอไม่ว่าจะยิงภาพใหม่กี่ครั้ง · ทางแก้ที่ใช้คือทำให้ลายเป็นลายน้ำจาง ไม่ใช่หดพื้นที่เนื้อหาของทุก deck — ตัวสร้างไฟล์ลดความเข้มให้อัตโนมัติที่ **หน้าพื้นขาว 40%** และ **หน้าพื้นเข้ม 75%** (หน้าพื้นเข้มจางน้อยกว่าเพราะตัวอักษรขาวบนพื้นเข้มมีความต่างสีสูงอยู่แล้ว) · ปรับรายเล่มได้ด้วยช่อง `background_opacity_light` และ `background_opacity_dark`

6. **ตรวจก่อนใช้ 3 ข้อ:** ลายกระจุกอยู่ขอบขวาและขอบล่าง ฝั่งซ้ายที่เป็นที่อยู่ของหัวเรื่องและเนื้อความต้องโล่ง · ความต่างของสีตัวอักษรกับพื้นผ่านเกณฑ์ (ขาวบนเข้ม / เทาเข้มบนขาว — เกณฑ์เดียวกับ D7.H3) · ทองคล้ำไม่ใช่ทองสด และเส้นบางจนไม่แย่งสายตาจากเนื้อหา
7. **FALLBACK ไม่มี Higgsfield/ยิงไม่ผ่าน:** วาดเองด้วย python-pptx (เส้นโค้ง/เรขาคณิตทอง freeform บาง ๆ ตาม motif) — **ไม่ block งาน ไม่วนรอ** แจ้ง user ว่าใช้ลายวาดแทนภาพ generate
