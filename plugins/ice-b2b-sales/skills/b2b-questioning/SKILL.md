...
- *Role:* AE (sales), SC (pre-sales / solution consultant), or CSM (customer success).
- *Stage:* prospecting, discovery, qualification, solution design, demo/proof, RFP, negotiation, close/handoff, onboarding, adoption, QBR, renewal, expansion, or escalation.
- *Persona:* the specific stakeholder(s) who will be in the room — Economic Buyer (usually CFO/CEO), Technical Buyer (CIO/IT Director), User Buyer (Finance Director, Ops Head, HR Director), or Champion.
- *Business process or solution area:* Record-to-Report, Procure-to-Pay, Order-to-Cash, Hire-to-Retire, Plan-to-Produce, Treasury, Compliance, Analytics, or cross-functional.
- *Cultural context:* Thai (default for this user), regional APAC, Western, or mixed. This determines register and indirection.
...
- **Prospecting / first meeting:** one SPIN-Situation opener, one Challenger question grounded in public research, one Champion-surfacing question.
- **Discovery (primary spine):** SPIN funnel — Situation → Problem → Implication → Need-Payoff — traversed once per business process. Use Sandler Pain Funnel layers 1–6 on the highest-priority pain. Use Five Whys when a problem statement is vague and root cause is unclear.
- **Qualification (private scorecard after the meeting):** MEDDPICC. Every letter below confidence 5/10 becomes the agenda for the next conversation.
- **Solution design workshop:** JTBD interview opener ("walk me through the last time you hired a system to do this job"), Design Thinking *How Might We* reframes, why/what/how laddering, Strategic Selling stakeholder map.
- **Demo / Proof:** discovery-driven scene scripting, open with context-question + close with Need-Payoff.
- **Negotiation:** open questions + disciplined silence after price.
- **QBR / Renewal:** accountability against committed success metrics, Socratic forward-looking questions, Champion health check.
- **Escalation / Churn risk:** direct truth-seeking questions, skeptic-surfacing.

See `references/frameworks.md` for the full treatment of each framework with enterprise software examples.

## Question taxonomy (twelve types)
...
## The ten mandatory elements of a great question

Every produced question must carry these elements. When reviewing a user's draft questions, score against this list:

1. *Purpose* — one-sentence statement of what this question is designed to produce.
2. *Tense* — deliberate location in past, present, or future.
3. *Scope* — calibrated altitude (not too broad, not too narrow).
4. *Specificity* — specific enough that only this customer can answer, not generic enough to apply anywhere.
5. *Neutrality* — no leading phrasing embedded.
6. *Cognitive load* — answerable in sixty seconds or less without forcing public long-form thought.
7. *Sequencing* — fits the funnel logic of the conversation.
8. *Follow-up* — at least one probing question drafted for the expected answer.
9. *Silence* — explicit reminder to hold three seconds after asking.
10. *Documentation* — plan to capture the answer in CRM, workshop minutes, or account notes.

Read `references/mandatory-elements.md` for the deep treatment with diagnostic tests.

## The twelve character traits of a master questioner
...
- **Account Executive:** prospecting → discovery → qualification → demo → negotiation → close/handoff.
- **Pre-Sales / Solution Consultant:** solution discovery workshop → fit-gap → RFP response → demo scripting → design validation.
- **Customer Success Manager:** onboarding → adoption reviews → QBR → renewal → expansion → escalation.
...
- `references/frameworks.md` — SPIN, MEDDPICC, Sandler Pain Funnel, Challenger, Five Whys, Socratic, Strategic Selling, JTBD, Design Thinking laddering. Read whenever a framework selection decision is needed or the user asks about any specific framework.
- `references/mandatory-elements.md` — the ten mandatory elements with diagnostic tests and funnel-sequencing discipline. Read when producing or scoring questions.
- `references/role-playbooks.md` — stage-by-stage agendas for AE, SC, CSM. Read when the user's role and stage are identified.
- `references/question-bank.md` — 125 tagged sample questions. Read when the user asks for examples, a starter set, or inspiration.
- `references/thai-context.md` — kreng jai, face, hierarchy, bunkhun calibration. Read whenever the cultural context is Thai or APAC.
- `references/anti-patterns-rubric.md` — 15 anti-patterns and the self-assessment rubric. Read when reviewing user drafts or transcripts.
- `references/design-thinking-complement.md` — empathy mapping, POV statements, stakeholder personas (from the companion Design Thinking research). Read during solution design workshops and co-creation sessions.
- `assets/playbook-full.md` — the complete source playbook as a single document, for deep reading when the user wants the full research.

## Invocation tips

- When the user's request is multi-stage (e.g., "prep me for a full discovery and negotiation cycle"), produce stage-sequenced outputs in one response, each with its own question set, rather than asking which stage to start with.
- When in doubt about whether Thai softening is needed, apply it — erring toward softened delivery rarely costs anything and often earns trust.
- When the user pastes a call transcript or rough notes, score the questions already asked against the ten mandatory elements, then propose the next three questions that the user should ask in the next conversation.
- When the user's context is government, e-bidding, or Thai public sector, cross-reference the `advisor-govt-gfmis` and `govt-egp-gfmis` skills if installed — questioning in regulated procurement has additional legal and process constraints.
- When the technology in scope is Oracle, SAP, NetSuite, or Salesforce, domain knowledge matters — depth of the question correlates directly with the user's technical fluency, so recommend pre-meeting homework if the user seems unsure of the technical baseline.
