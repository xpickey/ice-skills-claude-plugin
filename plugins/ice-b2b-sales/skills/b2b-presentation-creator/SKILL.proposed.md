...
| **D7 (รวม D7.7 และ D7.H8)** | มิติตรวจฟอนต์และการจัดหน้าของอริส ซึ่งเป็นด่านหยุดงานสำหรับไฟล์ที่ส่งลูกค้า |
...
หน้าปกไม่เกิน 10 คำ · หน้าคั่นไม่เกิน 8 คำ · หน้าอ้างคำพูดไม่เกิน 30 คำ · หน้าปิดไม่เกิน 10 คำ ที่มาของตัวเลขทั้งชุดอยู่ที่ `~/.claude/skills/b2b-slide-designer/references/design-principles.md` หัวข้องบความหนาแน่นของข้อมูล
...
รวบคำถามทั้งหมดถามครั้งเดียวก่อนเริ่มงาน อย่าทยอยถามระหว่างทาง
...
**เลือกรูปแบบผลลัพธ์** ข้อกำหนดชุดเดียวกันสร้างได้ทั้งสองรูปแบบ จึงถามให้ชัดก่อนลงมือ
...
|---|---|---|
| `ICE_BUILD=pipeline` | งานนี้เดินตามลำดับขั้นตอนของทีม ไม่ใช่การสร้างไฟล์นอกกระบวนการ | อ่านสกิล `ice-doc-builder` แล้วเดินขั้นตอนตั้งแต่ขั้นที่ 1 |
...
2. โหลดไฟล์ธีมจาก `assets/themes/` ตามธีมที่เลือกไว้ในขั้นที่ 3 และโหลดเลย์เอาต์จาก `assets/layouts/` ที่ตรงกับชนิด deck
3. รันคำสั่งสร้างไฟล์
   ```bash
   ICE_BUILD=pipeline ICE_DESIGN=briefed ICE_BASE=<ที่อยู่ไฟล์ฐาน|NEW> python scripts/build_deck.py \
...
5. ถ้าใช้แม่แบบสำเร็จรูปจากคลัง 71 framework ให้เรียกที่อยู่ไฟล์ด้วย `python b2b-slide-designer/scripts/template_cache.py get "<ชื่อ framework>"` · ถ้าได้สถานะ missing แปลว่าไฟล์แม่แบบไม่อยู่บนเครื่องนี้ ให้สร้างจากคลังภาพประกอบแทนแล้วแจ้ง user โดยไม่ปล่อยให้งานล้ม
...
| 3 | `python3 ~/.claude/agents/_lib/audit_fonts.py --rail private\|govt <ไฟล์>.pptx` | ชื่อฟอนต์ที่ใช้จริงในไฟล์ ตรงตามนโยบายฟอนต์ของงานเอกชนหรืองานราชการ | ผ่านทั้งไฟล์ |
| 4 | `_lib/validate_pptx_fonts.py` (เครื่องมือของสายสร้างไฟล์) | ฟอนต์ถูกฝังมากับไฟล์จริง สำหรับงานที่ถึงมือลูกค้า | ผล PASS |

การตรวจภาษาของข้อความบนสไลด์ รวมทั้งตัวตรวจภาษาที่ต้องรัน อยู่ที่สกิล `ice-writing-register` หัวข้อวิธีใช้สำหรับ agent ซึ่งเป็นบ้านเดียวของเรื่องภาษา
...
2. โครงเรื่องรูปแบบ markdown ที่ `outline_V##R##.md`
