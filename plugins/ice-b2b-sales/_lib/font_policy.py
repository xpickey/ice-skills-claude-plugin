...
V01R08 | 2026.08.05
  V01R08 (คำสั่ง user · เคส VFIN) งานต่อยอด template เดิม = ใช้ฟอนต์ตามนโยบายปัจจุบัน
         ห้ามสืบทอดฟอนต์ template อัตโนมัติ — ต้องมีคำสั่ง user ชัดเจน (font_override_reason)
...
#   • ไฟล์ที่ลูกค้า/ผู้ขายส่งมา (เราตรวจ)  → ไม่ใช่ของเราสร้าง ไม่อยู่ใต้นโยบายเรา
#
# 🔴 V01R08 (คำสั่ง user 2026.08.05 · เคส VFIN MA-AMS-CR) — สิ่งที่ **ไม่ใช่** ข้อยกเว้นอีกต่อไป:
#   • งานที่เราสร้าง**ต่อยอด template/เด็คเดิม** → ใช้ฟอนต์ตามนโยบายปัจจุบันเป็นค่าเริ่มต้น
#     **ห้ามสืบทอดฟอนต์ของ template โดยอัตโนมัติ** — agent ห้ามออก --allow-font ให้ตัวเอง
#     ด้วยเหตุผล "ตามความสม่ำเสมอของเด็คเดิม"
#   • ฟอนต์ template ใช้ได้**เฉพาะเมื่อ user สั่งชัดเจน** ("ใช้ font ตาม template") →
#     บันทึก spec["font_override_reason"] = "user สั่งใช้ font ตาม template" + จดใน QA-log
#   • ผลข้างเคียงที่ต้องแจ้ง user ใน PLAN-CARD: สไลด์/หน้าใหม่จะฟอนต์ต่างจากของเดิมในชุดเดียวกัน
#     (mixed-font ชั่วคราว) จนกว่าเด็คทั้งใบจะ migrate — user เลือกได้ว่ารับ หรือสั่งใช้ font template
EXTERNAL_MANDATE_NOTE = ("TOR/e-GP · ข้อบังคับวารสาร-มหาวิทยาลัย · ไฟล์รับมาจากภายนอกที่เราตรวจ "
                         "= อยู่นอกนโยบายนี้ ใช้ --allow-font · "
                         "⚠ งานต่อยอด template เดิม ไม่ใช่ข้อยกเว้นอัตโนมัติ — "
                         "ฟอนต์ template ต้องมีคำสั่ง user ชัดเจนเท่านั้น (font_override_reason)")
