...
สิ่งที่ตรวจ (แต่ละข้อรายงานตำแหน่งและข้อความ):
  ...
  T7 คำต้องห้ามงานธุรกิจ       zero-risk best-in-class world-class · คำละคร องก์ ฉาก ตัวละคร · ระบบเป็นตัวละคร (ระบบไม่คุยกัน) · อ้างสัมบูรณ์ (แม้แต่วันเดียว)
...
T3 = re.compile(r"(?<!วัน)(ทำการ|มีการ)(?=[ก-๙])|ในส่วนของ|ทางด้าน(?=[ก-๙])")  # "วันทำการ" คือวันทำงาน ไม่ใช่คำเติม
...
# T7 เพิ่ม 2026.09.06 จากคำทักของ user ต่อ deck ซ้อมจริง: (ก) เอาระบบมาเป็นตัวละคร "ระบบไม่คุยกัน" = ลิเก
# (ข) คำอ้างสัมบูรณ์ที่ขายเกินจริง "ธุรกิจไม่หยุดแม้แต่วันเดียว" — ทั้งสองแบบผ่านตัวตรวจเดิมได้ จึงต้องมีแบบแผนเฉพาะ
T7_PERSONIFY = re.compile(r"ระบบ[ก-๙ ]{0,12}?(คุยกัน|ไม่คุยกัน|พูดกัน|รู้ใจ|เข้าใจกัน|จับมือกัน|ทำงานเป็นเพื่อน)")
T7_ABSOLUTE = re.compile(r"แม้แต่วันเดียว|แม้วันเดียว|แม้แต่นาทีเดียว|ไม่มีวัน|ตลอดไป|ตลอดกาล|ไม่มีทาง(?:ผิด|พลาด|ล้ม)|ไร้ที่ติ|ไม่มีข้อผิดพลาด(?:เลย)?|ร้อยเปอร์เซ็นต์|100 ?%\s*(?:ทุก|ตลอด)|ทุกอย่าง(?:ครบ)?100")
...
def check(text, register="business"):
    ...
    if register == "business":
        ...
        for m in T7_PERSONIFY.finditer(text):
